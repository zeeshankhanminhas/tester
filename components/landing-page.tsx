...
function ExpeditionGrid({ expeditions, cardRefs }: { expeditions: Expedition[]; cardRefs: MutableRefObject<Array<HTMLElement | null>> }) {
  return (
    <section id="dossiers" className="section-shell py-20 sm:py-32">
      ...
      <div className="grid gap-6 lg:grid-cols-4 lg:grid-rows-[360px_360px]">
        {expeditions.map((expedition, index) => {
          ...
          return (
            <article key={expedition.id} ref={(node) => { cardRefs.current[index] = node; }} className={`group relative overflow-hidden border border-white/10 bg-obsidian shadow-dossier ${featured ? "min-h-[650px] lg:col-span-2 lg:row-span-2" : "min-h-[520px] lg:col-span-2 lg:min-h-0"}`}>
